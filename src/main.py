...
def run(ticker="AAPL", period="5y", force_retrain=False):
    ...
    result = {
        ...
        "model_predicted_return": float(prediction_info.get("model_predicted_return", prediction_info["predicted_return"])),
        "decision_return": float(prediction_info.get("decision_return", prediction_info["predicted_return"])),
        "decision": decision,
        "decision_source": prediction_info.get("decision_source", "regression-quantile"),
        "classifier_decision": prediction_info.get("classifier_decision"),
        "decision_thresholds": prediction_info.get("decision_thresholds"),
